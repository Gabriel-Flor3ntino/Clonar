...
public class VooController {
    ...
    public void createVoo(int nroVoo, String data, String hora, int aviaoIndex) {
        Aviao aviao = aviaoController.getAviao(aviaoIndex);
        if (aviao != null) {
            Voo voo = new Voo(aviao, nroVoo, data, hora);
            voos.add(voo);
        }
    }
    ...
    public void updateVoo(int index, int nroVoo, String data, String hora, int aviaoIndex) {
        if (index >= 0 && index < voos.size()) {
            Voo voo = voos.get(index);
            Aviao aviao = aviaoController.getAviao(aviaoIndex);
            if (aviao != null) {
                voo.setNroVoo(nroVoo);
                voo.setData(data);
                voo.setHora(hora);
